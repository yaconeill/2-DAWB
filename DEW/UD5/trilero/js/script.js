// import createElement from 'functionCollection';
// import shuffle from 'functionCollection';
import * as own from 'functionCollection';
// Amount of cards
var numCards = 3;
...
function elementsCreation() {
    ...
    own.createElement('h1', body, 'Trileros');
    own.createElement('select', body.firstElementChild, null, 'id', 'select');

    var select = document.getElementById('select');
    own.createElement('option', select, 'Dificultad', 'id', 'difficulty');
    for (let j = 3; j <= 5; j++) {
        own.createElement('option', select, j + ' cartas', 'value', j, 'id', j);
    }

    own.createElement('div', body, null, 'id', 'container');
    var container = document.getElementById('container');
    own.createElement('div', container, null, 'class', 'board');
    ...
    for (let i = 0; i < numCards; i++) {
        own.createElement('div', board, null, 'class', 'cards');
        ...
        if (img[i].includes('Asdeoros'))
            fig = 'figure';
        own.createElement('div', cards, null, 'class', `back card-${i+1} ${fig}`);
        own.createElement('div', cards, null, 'class', `front card-${i+1} ${fig}`);
    ...
    own.createElement('div', body, null, 'class', 'footer');
    var footer = body.lastElementChild;
    own.createElement('p', footer, null, 'id', 'text');
    own.createElement('button', footer, 'Otra vez', 'id', 'button');
}
...
//  */
own. // function createElement(tag, node, text, attr1, attrValue1, attr2, attrValue2, attr3, attrValue3) {
own. // 	var element = document.createElement(tag);
    // 	if (attr1 != undefined) {
